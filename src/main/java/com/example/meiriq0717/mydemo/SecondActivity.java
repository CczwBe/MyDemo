...
public class SecondActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.secondacitivy_layout);
        // initView();
        newInit();
    }

    private void newInit() {
        mListView = (ListView) findViewById(R.id.sec_activity_lv);
        initData();
        ImageLoaderFactory.customizeCache(SecondActivity.this, 1024 * 10, Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).getAbsolutePath(), 10 * 1024);
        DefaultImageLoadHandler handler = new DefaultImageLoadHandler(SecondActivity.this);
        ...
        ImageLoader imageLoader = ImageLoaderFactory.create(SecondActivity.this,handler);
        ImageLoaderFactory.setDefaultImageLoadHandler(handler);
        imageLoader.setImageLoadHandler(handler);
       // imageLoader.setImageLoadHandler(new DefaultImageLoadHandler(SecondActivity.this));
//        ((DefaultImageLoadHandler) imageLoader.getImageLoadHandler()).setImageRounded(true, LocalDisplay.dp2px(50));
        MAdapter adapter = new MAdapter();
        adapter.setViewHolderClass(this, MViewHolder.class, imageLoader);
        adapter.setList(mList);
        Log.i("ArrayList", mList.toString());
    ...
    private void initData() {
        for (int index = 0; index < imageUrls.length; index++) {
            ...
            map.put("image_url", imageUrls[index]);
            map.put("title", "imageTitle" + index);
            mList.add(map);
        }
//        mAdapter = new MyAdapter(mList, SecondActivity.this);
//        mListView.setAdapter(mAdapter);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.fab:
                ImageProvider mImageProvider = ImageLoaderFactory.getDefaultImageProvider(SecondActivity.this);
//                mImageProvider.clearDiskCache();
//                mImageProvider.clearMemoryCache();
                long l = mImageProvider.getMemoryCacheUsedSpace() / 1024 / 1024;
                Long l2 = mImageProvider.getFileCacheUsedSpace() / 1024 / 1024;
                String cachePath = mImageProvider.getFileCachePath();

                if (firstClick) {
                    firstClick = false;
                    mImageProvider.clearDiskCache();
                    mImageProvider.clearMemoryCache();
                    Toast.makeText(SecondActivity.this, " 缓存已清除", Toast.LENGTH_SHORT).show();

                } else {
                    firstClick = true;
                    Toast.makeText(SecondActivity.this, " 已用内存缓存 " + l + " 文件缓存" + l2 + "文件缓存路径" + cachePath, Toast.LENGTH_SHORT).show();
                }
